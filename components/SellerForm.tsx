...
const initial: FormData = {
  companyName: '', representativeName: '', birthDate: '', postalCode: '',
  address: '', phone: '', email: '', website: '', industry: '', revenue: '',
  profit: '', employees: '', founded: '', bank: '', fixedAssets: '', totalDebt: '',
}
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Field label="年商" required error={errors.revenue}>
          <select value={form.revenue} onChange={set('revenue')} className={inputCls(errors.revenue)}>
            <option value="">選択してください</option>
            {revenueOptions.map(r => <option key={r} value={r}>{r}</option>)}
          </select>
        </Field>
        <Field label="営業利益" required error={errors.profit}>
          <select value={form.profit} onChange={set('profit')} className={inputCls(errors.profit)}>
            <option value="">選択してください</option>
            {profitOptions.map(p => <option key={p} value={p}>{p}</option>)}
          </select>
        </Field>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Field label="従業員数">
          <input
            type="number" value={form.employees} onChange={set('employees')}
            className={inputCls()} placeholder="10" min="0"
          />
        </Field>
        <Field label="創業年度（西暦）">
          <input
            type="number" value={form.founded} onChange={set('founded')}
            className={inputCls()} placeholder="2000" min="1900" max="2025"
          />
        </Field>
        <Field label="取引金融機関">
          <input
            type="text" value={form.bank} onChange={set('bank')}
            className={inputCls()} placeholder="〇〇銀行"
          />
        </Field>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Field label="固定資産額（概算）">
          <input
            type="text" value={form.fixedAssets} onChange={set('fixedAssets')}
            className={inputCls()} placeholder="例：3,000万円"
          />
        </Field>
        <Field label="負債総額（概算）">
          <input
            type="text" value={form.totalDebt} onChange={set('totalDebt')}
            className={inputCls()} placeholder="例：1,500万円"
          />
        </Field>
      </div>
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full bg-[#C8A96E] hover:bg-[#b8976a] disabled:opacity-60 text-white font-bold py-4 px-8 rounded-lg text-lg transition-all duration-200 shadow-lg hover:shadow-xl mt-4"
      >
        {loading ? '送信中...' : '売り手リストに掲載を申し込む（無料）'}
      </button>
